    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-darkbg-950/95 border border-darkbg-700/60 p-3 rounded-lg shadow-glass backdrop-blur-md">
          <p className="text-gray-200 font-semibold text-xs mb-1.5">{payload[0].payload.name}</p>
          <div className="flex flex-col gap-1 text-xxs">
            <span className="text-emerald-400">
              You: <strong className="text-gray-100 text-sm font-semibold">{payload[0].value}</strong> kg CO₂
            </span>
            <span className="text-cyan-400">
              Baseline: <strong className="text-gray-100 text-sm font-semibold">{payload[1].value}</strong> kg CO₂
            </span>
          </div>
        </div>
      );
